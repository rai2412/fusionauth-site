...
- <Field label="Project"/>: `Mavern`
- <Field label="Language"/>: `Java`
- <Field label="Spring Boot Version"/>: `2.7.16`
- Project Metadata
  - <Field label="Group"/>`io.fusionauth.quickstart`
  - <Field label="Artifact"/>`springweb`
  - <Field label="Package Name"/>`io.fusionauth.quickstart.springweb`
  - <Field label="Packaging"/>`Jar`
  - <Field label="Java Version"/>`17`
...
- <Field label="Dependencies"/>`OAuth2 Client`
- <Field label="Dependencies"/>`Thymeleaf`
- <Field label="Dependencies"/>`Spring Web`
...
* You may want to customize the [token expiration times and policies](/docs/lifecycle/authenticate-users/oauth/) in FusionAuth.
...
* Set up [MFA](/docs/lifecycle/authenticate-users/multi-factor-authentication), [Social login](/docs/lifecycle/authenticate-users/identity-providers/), or [SAML](/docs/lifecycle/authenticate-users/identity-providers/) integrations.
* Integrate with external systems using [Webhooks](/docs/extend/events-and-webhooks/events/), [SCIM](/docs/lifecycle/migrate-users/scim/scim), and [Lambdas](/docs/extend/code/lambdas/).
