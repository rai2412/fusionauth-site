...
import UserInfoResponse from 'src/content/docs/_shared/_user-info-response.mdx';
...
The following response applies to the <ScrollRef target="Complete the Authorization Code Grant Request" />, <ScrollRef target="Refresh Token Grant Request" /> and <ScrollRef target="Complete the Device Authorization Grant Request" /> examples.
...
<APIBlock>
  ...
  <APIField name="scope" type="String" optional>
    ...
    * `openid` - This scope is used to request an `id_token` be returned in the response
    * `offline_access` - This scope is used to request a `refresh_token` be returned in the eventual <ScrollRef target="Token" /> response.
...
This endpoint is equivalent to the <ScrollRef target="Device Validate" /> endpoint, but it will return meta-data for the <InlineField>user_code</InlineField> in a JSON body.
